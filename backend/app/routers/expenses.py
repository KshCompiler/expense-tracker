...
def create_expense(payload: ExpenseIn, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    amount, date = validate_transaction_input(payload.amount, payload.category, VALID_CATEGORIES, payload.date, "category")
    return crud.add_expense(db, current_user.id, amount, payload.category, date, payload.description)
...
def edit_expense(
    ...
    amount, date = validate_transaction_input(payload.amount, payload.category, VALID_CATEGORIES, payload.date, "category")
    return crud.update_expense(db, expense_id, amount, payload.category, date, payload.description)
